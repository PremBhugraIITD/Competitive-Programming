...
void binary_lifting(ll curr, ll p, vector<vll> &adj, vector<vll> &parent, vector<vll> &minimum, vll &depth, map<pll, ll> &weight)
{
    if (p != 0)
    {
    ...
    if (p != 0)
    {
        minimum[curr][0] = weight[{curr, p}];
    }
    else
    {
        minimum[curr][0] = INT_MAX;
    }
    fr(i, 1, 18)
    {
    ...
    for (auto &neighbor : adj[curr])
    {
        if (neighbor != p)
        {
            binary_lifting(neighbor, curr, adj, parent, minimum, depth, weight);
        }
    }
}
...
int main()
{
    ...
    while (t--)
    {
        ...
        vvll minimum(n + 1, 18, LLONG_MAX);
        vll depth(n + 1, 0);
        binary_lifting(1, 0, adj, parent, minimum, depth, weight);
        ll q;
        cin >> q;
        fr(i, 1, q)
        {
