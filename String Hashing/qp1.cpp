...
public:
    ...
    Hash(string s)
    {
        ...
        fr(i, 1, n)
        {
            ...
            prefixHash1[i] = (prefixHash1[i - 1] + ((s[i] - 'a' + 1) * pow1[i]) % m1) % m1;
            prefixHash2[i] = (prefixHash2[i - 1] + ((s[i] - 'a' + 1) * pow2[i]) % m2) % m2;
...
int main()
{
    ...
    else
    {
        ...
        Hash H1(s);
        Hash H2(p);
        pll target = H2.getHash(0, m - 1);
        fr(i, 0, n - m + 1)
        {
            pll temp = H1.getHash(i, i + m - 1);
            if (temp == target)
            {
                count++;
            }
